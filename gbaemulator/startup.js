const gba_rom_image_supermario2 = document.getElementById("game-rom-image-supermario2");
const gba_rom_image_kirby_mirror = document.getElementById("game-rom-image-kirby-mirror");
const gba_rom_image_metroid_fusion = document.getElementById("game-rom-image-metroid-fusion");
const gba_rom_image_pokemon_emerald = document.getElementById("game-rom-image-pokemon-emerald");

var gba;
var runCommands = [];
var debug = null;

//The following on click methods opens a http get request to fetch
//the file that contains the named game, and runs it.
gba_rom_image_kirby_mirror.onclick = () => {
    var request = new XMLHttpRequest();
    request.open('GET', './roms/Kirby & the Amazing Mirror (USA).gba');
...
gba_rom_image_metroid_fusion.onclick = () => {
    var request = new XMLHttpRequest();
    request.open('GET', './roms/Metroid Fusion (USA, Australia).gba');
    request.responseType = 'blob';
    request.onload = function() {
        run(request.response);
    };
...
gba_rom_image_pokemon_emerald.onclick = () => {
    var request = new XMLHttpRequest();
    request.open('GET', './roms/Pokemon - Emerald Version (USA, Europe).gba');
    request.responseType = 'blob';
    request.onload = function() {
        run(request.response);
    };
...
gba_rom_image_supermario2.onclick = () => {
    var request = new XMLHttpRequest();
    request.open('GET', './roms/Super Mario Advance 2 - Super Mario World (USA, Australia).gba');
    request.responseType = 'blob';
    request.onload = function() {
        run(request.response);
    };
